export const RegisterConfig = Object.freeze({
  typeGood:'success',    
  typeBad:'danger',    
  messageGood:'Se envio un correo de recuperacion exitosamente.',    
  messageBad:'Se presento un error por favor intentelo nuevamente.',

  






  enail: 'Correo electrónico:',
  password: 'Contraseña:',
  emailRequired: 'Falta el correo electrónico',
  emailInvalid: 'Formato de correo electrónico no valido',
  passwordInvalid: 'Falta la contraseña',
  btnRegister: 'Registrarse',
  messageLogin: '¿ Ya tienes cuenta ?',
  linkLogin: 'Iniciar sesión',
  title: 'Registrate en Amore Pizza',
  DNI: 'DNI',
  genericPending: 'Este campo es requerido',
  name: 'Nombre',
  lastName: 'Apellidos',
  phone: 'Teléfono',
  address: 'Dirección',
  errorPhone: 'Teléfono invalido',
  refered: 'Codigo referido'

});
